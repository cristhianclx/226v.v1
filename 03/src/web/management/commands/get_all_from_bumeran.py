...
class Command(BaseCommand):
    ...
    def obtainData(self):
        ...
        raw_from_bumeran = requests.post(url, headers={
        ...
        for x in raw_from_bumeran["content"]:
            data.append({
                "FECHA_SCRAP": "{}".format(datetime.now().strftime("%d/%m/%Y %H:%M")),
                "CATEGORIA": "",
                "FUNCION": "",
                "EMPRESA": x["empresa"],
                "PUESTO": x["titulo"],
    ...
    def handle(self, *args, **options):
        ...
        job_raw['DESC_LIMPIO'] = job_raw.apply(lambda row: wrangler(row['DESC_PUESTO']), axis=1)
